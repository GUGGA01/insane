...
# INSANE PAPA user IDs
INSANE PAPA_id = ["1221262658"]
...
def show_help(message):
 ...
 To See INSANE PAPA Commands:
 /INSANE PAPAcmd : Shows All INSANE PAPA Commands.
 By @iNSANE_010
'''
    for handler in bot.message_handlers:
        if hasattr(handler, 'commands'):
            ...
            elif handler.doc and 'INSANE PAPA' in handler.doc.lower():
                continue
            else:
                help_text += f"{handler.commands[0]}: {handler.doc}\n"
...
@bot.message_handler(commands=['INSANE PAPAcmd'])
def welcome_plan(message):
    user_name = message.from_user.first_name
    response = f'''{user_name}, INSANE PAPA Commands Are Here!!:
